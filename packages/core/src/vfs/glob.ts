import type { WasmGlobModule } from "../wasm-interfaces.js";
...
function doMatch(pattern: string, pi: number, str: string, si: number): boolean {
	while (pi < pattern.length) {
		...
		if (ch === "*" && pi + 1 < pattern.length && pattern[pi + 1] === "*") {
			// Consume the **
			pi += 2;
			// If ** is followed by /, skip it
			if (pi < pattern.length && pattern[pi] === "/") {
				pi++;
			}

			// If nothing left in pattern, it matches everything
			if (pi >= pattern.length) return true;

			// Try matching the rest of the pattern at every subsequent position
		...
		if (ch === "*") {
			pi++;
			// Try matching zero or more non-slash chars
			for (let i = si; i <= str.length; i++) {
		...
		if (ch === "[") {
			...
			const matched = matchCharClass(chars, str[si]);
...
function matchCharClass(classContent: string, ch: string): boolean {
	...
	while (i < classContent.length) {
		...
		if (i + 2 < classContent.length && classContent[i + 1] === "-") {
			...
			if (ch >= current && ch <= rangeEnd) return true;
			i += 3;
		} else {
