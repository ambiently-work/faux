...
import type { VfsNode, NodeMeta } from "./node.js";
import { createFile, createDirectory, createSymlink, defaultMeta } from "./node.js";
import { resolve, normalize, dirname, basename, split, isAbsolute, join } from "./path.js";
...
export class VirtualFileSystem implements IFileSystem {
	...
	constructor(options?: VirtualFileSystemOptions) {
		...
		for (const dir of ["/tmp", "/root", "/home", "/usr/bin", "/bin", "/dev", "/var"]) {
	...
	private walkPath(absPath: string, followSymlinks: boolean, hops: number): VfsNode | null {
		...
		for (let i = 0; i < parts.length; i++) {
			...
			if (child.kind === "symlink" && (followSymlinks || i < parts.length - 1)) {
				...
				// Continue walking from the resolved node
	 ...
	 * Creates no directories — just resolves the parent path.
	 */
	...
	private checkExecute(node: VfsNode, path: string): void {
		const perm = this.getEffectivePermBits(node);
		if (!(perm & 1)) throw eacces(path);
	}

	/**
	 * Get the rwx bits relevant to the current uid/gid.
	 * Returns a 3-bit number (rwx).
	 */
	...
	writeFile(path: string, content: string): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].writeFile(mounted[1], content); return; }
	...
	appendFile(path: string, content: string): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].appendFile(mounted[1], content); return; }
		try {
	...
	mkdir(path: string, options?: { recursive?: boolean }): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].mkdir(mounted[1], options); return; }
	...
	rm(path: string, options?: { recursive?: boolean; force?: boolean }): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].rm(mounted[1], options); return; }
		...
		// For symlinks, don't follow — remove the link itself
	...
	cp(src: string, dest: string, options?: { recursive?: boolean }): void {
		...
		// Cross-mount or mount-to-local: read from source, write to dest
		// Use readFile/writeFile which already handle mount delegation
		const content = this.readFile(absSrc);
		const destStat = (() => { try { return this.stat(absDest); } catch { return null; } })();
	...
	private copyDirRecursive(srcDir: string, destDir: string): void {
		const srcNode = this.resolveNode(srcDir);
		if (!srcNode || srcNode.kind !== "directory") throw enoent(srcDir);
		...
		for (const [name, child] of srcNode.children) {
			...
			if (child.kind === "directory") {
				this.copyDirRecursive(childSrc, childDest);
			} else if (child.kind === "file") {
				this.writeFile(childDest, child.content);
			} else if (child.kind === "symlink") {
				this.symlink(child.target, childDest);
			}
		}
	}
	...
	mv(src: string, dest: string): void {
		...
		// Cross-mount mv: copy then remove
	...
	chmod(path: string, mode: number): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].chmod(mounted[1], mode); return; }
		const node = this.resolveNode(absPath);
		if (!node) throw enoent(absPath);
		node.meta.mode = mode;
	...
	chown(path: string, uid: number, gid: number): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].chown(mounted[1], uid, gid); return; }
		const node = this.resolveNode(absPath);
		if (!node) throw enoent(absPath);
		node.meta.uid = uid;
		node.meta.gid = gid;
	...
	symlink(target: string, path: string): void {
		const absPath = this.resolvePath(path);
		const mounted = this.getMountedFs(absPath);
		if (mounted) { mounted[0].symlink(target, mounted[1]); return; }
		const [parent, name] = this.resolveParent(absPath);
		this.checkWrite(parent, dirname(absPath));
	...
	private resolveRealPath(absPath: string, hops: number): string {
		...
		for (const part of parts) {
			...
			if (node.kind === "symlink") {
				const target = isAbsolute(node.target)
					? node.target
					: resolve(resolved, node.target);
	...
	glob(pattern: string, options?: { cwd?: string }): string[] {
		...
		// Check if the pattern falls under a mount
		const mounted = this.getMountedFs(absPattern);
		if (mounted) {
			// Delegate glob to the mounted FS, then re-prefix paths
		...
		// Also collect paths from all mounts
	...
	mount(mountPoint: string, fs: IFileSystem): void {
		...
		// Ensure the mount point directory exists in the VFS tree
	 ...
	 * Returns [mountedFs, relativePath] or null if no mount matches.
	 * Finds the longest (most specific) matching mount point.
	 */
	...
	/**
	 * Dump all files as a flat Record<string, string>.
	 */
