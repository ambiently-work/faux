 ...
 *   - listEndpoint: URL that returns a JSON array of file paths for readDir
 *   - cache: whether to cache fetched files in memory (default: true)
 */
...
export class HttpFileSystem implements IFileSystem {
	...
	private fetchSync(path: string): string {
		...
		// we throw if the file hasn't been pre-fetched.
		// Users should call prefetch() or use the async adapter.
		throw new Error(
	 ...
	 * Call this before using readFile/stat/etc.
	 */
	...
	/**
	 * Pre-fetch the directory listing (if listEndpoint is configured).
	 */
